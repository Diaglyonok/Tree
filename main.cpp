...
int Tree::inside(Node* child, int& count)
{
	if(child)
	{
		...
		cout << child->d << ' ';
		if(child->rNode==NULL || child->mNode==NULL || child->lNode==NULL) {count++;}
	}
	return 0;
}
	...
	do
	{
		...
		if(iMenu)
		{
			...
			if(tree.exist())
			{
				...
				cout<<endl<<"Количество вершин, имеющих не более 2-х потомков: "<<count<<endl;
				cin.clear();
				cin.sync();
				cin.get();
			}
...
//***********************************************************************
//Функция, выводящая меню на экран
int menu()
{
	int point;
	do{
